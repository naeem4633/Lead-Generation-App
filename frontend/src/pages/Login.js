    ...
    <section className='w-full relative tracking-wide min-h-[80vh] flex justify-center items-center flex-col'>
      <form className="authorization-form">
        ...
        <div className="authorization-inputForm">
          ...
          <input type="email" id="email" name="email" className="authorization-input" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your Email"/>
        </div>
        ...
        <div className="authorization-inputForm">
          ...
          <input type="password" id="password" name="password" className="authorization-input" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your Password"/>
          ...
          <button type="button" onClick={handleLogin} className="authorization-button-submit transition-all duration-200">Sign In</button>
          ...
          <div className='flex flex-col'>
            <div className='w-full flex flex-row justify-center items-center space-x-4'>
              <div className="w-full authorization-flex-row">
                <button className="authorization-btn google" onClick={handleLoginWithGoogle}>
              ...
              <div className="w-full authorization-flex-row">
                {!user && (<button className="authorization-btn google" onClick={handleAnonymousLogin}>
                  Guest Login 
                </button>)}
                {user && user.isAnonymous && (<span className='select-none p-4' style={{ color: 'red', fontSize: '12px' }}> You're already logged in</span>)}
              </div>
            </div>
          </div>
        </form>
